...
//            });
    ...
    $(options.videos).each(function loopingItems()
    {
		...
		if(self.item!=undefined){
			...
			{
				if(self.scrollingIsOn & self.hasTouch)
					return;
				if(self.preloader)
        ...
        switch(self.options.playlist){
            ...
            case "Bottom playlist":
                ...
                {
					$(self.item_array[0]).removeClass("elite_vp_itemUnselected_bottom"+" "+this.options.instanceTheme).addClass("elite_vp_itemSelected_bottom"+" "+this.options.instanceTheme);//first selected
					self.item_array[0].find(".elite_vp_thumbnail_image").removeClass("elite_vp_thumbnail_image").addClass("elite_vp_thumbnail_imageSelected");// selected
                }
                break;
        }
