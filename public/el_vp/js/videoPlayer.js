		...
		success: function(data) {
			...
			$.each(data.items, function(i, item) {
				...
				var obj=
				{
					...
					prerollAD:self.yt_ads_array[self.id].prerollAD,
					prerollGotoLink:self.yt_ads_array[self.id].prerollGotoLink,
					preroll_mp4:self.yt_ads_array[self.id].preroll_mp4,
					prerollSkipTimer:self.yt_ads_array[self.id].prerollSkipTimer,
					midrollAD:self.yt_ads_array[self.id].midrollAD,
					midrollAD_displayTime:self.yt_ads_array[self.id].midrollAD_displayTime,
					midrollGotoLink:self.yt_ads_array[self.id].midrollGotoLink,
					midroll_mp4:self.yt_ads_array[self.id].midroll_mp4,
					midrollSkipTimer:self.yt_ads_array[self.id].midrollSkipTimer,
					postrollAD:self.yt_ads_array[self.id].postrollAD,
					postrollGotoLink:self.yt_ads_array[self.id].postrollGotoLink,
					postroll_mp4:self.yt_ads_array[self.id].postroll_mp4,
					postrollSkipTimer:self.yt_ads_array[self.id].postrollSkipTimer,
					popupAdShow:self.yt_ads_array[self.id].popupAdShow,
					popupImg:self.yt_ads_array[self.id].popupImg,
					popupAdStartTime:self.yt_ads_array[self.id].popupAdStartTime,
					popupAdEndTime:self.yt_ads_array[self.id].popupAdEndTime,
					popupAdGoToLink:self.yt_ads_array[self.id].popupAdGoToLink,
    ...
    if(self.options.playerLayout == "fitToContainer" || self.options.playerLayout == "fitToBrowser")
    {
		...
		if (self.agentID && (self._playlist.videos_array[this._playlist.videoid].videoType=="HTML5" || self.options.videoType=="HTML5 (self-hosted)"))
		{
			if(!self.options.showAllControls){
				...
				self.videoElement.height(self.element.width());
				self.videoElementAD.width(self.element.width());
				self.videoElementAD.height(self.element.width());
			}
		}
...
Video.fn.createVideoAdTitleInsideAD = function(){
    ...
    this.videoAdBoxInside.append('<div class="elite_vp_adsTitleInside">' + "Advertisement" + '</div>');
...
Video.fn.setupElement = function()
{
	...
	// this.element.append('<div class="chanads"></div>');
    ...
    this.progressADBg.append(this.progressAD);
		...
		$(this.mainContainer).find(".elite_vp_playerElement").bind("mousemove mouseenter click", function(e){
			...
			if ($(this).hasClass("elite_vp_videoTrack"+" "+self.options.instanceTheme)){
				...
				self.toolTip.text("Download video");
				self.toolTip.css("left", x+$(this).position().left);
				self.toolTip.css("top", self.controls.position().top - self.toolTip.outerHeight() - 2);
				self.toolTip.show();
			}
