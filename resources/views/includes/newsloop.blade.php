...
<title>African Entertainment News, TV Business news, Celebrity News | News</title>
<meta name="description" content="Wizkid style. Davido and Orange. Get latest Africa entertainment news, and all the hottest stories in African entertainment around the globe!" />
...
<div class="home-pro" >
        ...
        <div class="newsbox nopadding">
            ...
            <div class="nb-3 hidden-sm hidden-xs" >
                <div class="text-centre">
                    <script data-cfasync="false" type="text/javascript" src="https://www.liveadexchanger.com/a/display.php?r=1782619"></script>
                </div>
    ...
    c3.1-0.5,6.3-0.7,9.6-0.7c37.3,0,67.6,30.4,67.6,67.7C480,303,449.7,333.3,412.4,333.3z" />
                    ...
                    <div class="col-md-4 no-padding" style="padding: 1px;">
                        <div class="weat_area text-center">
                            <h6 style="color: #dadada; margin: 0; padding-top: 4px; font-size: 12px" id="weat_location"></h6>
...
<section class="" style="background-color: #dadada; padding: 40px 0px 30px 0px">

    <div class="container">
        <div class="row">
            <div class="col-lg-12 col-md-12">
                <!-- Nav tabs --><div class="card">
                    <div class="news_scroll_menu">

                        <ul class="nav nav-tabs" role="tablist">
                            @foreach($news as $n_key => $n_val)
                            @if($n_key == 0)
                                <li role="presentation" class="active"><a href="#slide___{{$n_val->id}}" aria-controls="{{$n_val->name}}" role="tab" data-toggle="tab">{{$n_val->name}}</a></li>
                            @continue
                            @endif
                                <li role="presentation"><a href="#slide___{{$n_val->id}}" aria-controls="{{$n_val->name}}" role="tab" data-toggle="tab">{{$n_val->name}}</a></li>
                            @endforeach
                        </ul>
                    ...
                    <div class="tab-content">
                        ...
                        @foreach($news as $n_key => $n_val)
                            ...
                            <div role="tabpanel" class="tab-pane <?=$n_key == 0 ? 'active':''; ?>" id="slide___{{$n_val->id}}">
                                <div class="row">
                                @foreach($n_val->posts as $post_key => $post)
                                    @if($post_key == 0)

                                    <a href="/news/{{$post->slug}}">
                                        <div class="col-md-7">
                                            <img src="{{ asset('assets/images/' . $post->image) }}" alt="{{$post->title}}">
                                            <h4>{{ substr(strip_tags($post->title), 0, 100 ) }}{{ strlen(strip_tags($post->title)) > 100 ? "..." : ""  }}</h4>
                                        </div>
                                    </a>
                                    ...
                                    <div class="col-md-5">
                                        <a href="/news/{{$post->slug}}"><p style="font-size: 18px;margin: 0 0 11px;color: #444; font-weight: 500">{{ substr(strip_tags($post->title), 0, 100 ) }}{{ strlen(strip_tags($post->title)) > 100 ? "..." : ""  }}</p></a>
                                    </div>
                                ...
                                <div class="row n_l_area">
                                    <hr style="color: rgb(161, 178, 189) !important;">
                                    @foreach($n_val->posts as $post_key => $post)
                                        @if($post_key == 0)
                                        @continue
                                        @endif
                                        ...
                                        <a href="/news/{{$post->slug}}">
                                            <div class="col-md-6" style="margin-top: 20px">
                                                ...
                                                <div class="col-md-6">
                                                    ...
                                                    <p style="font-size: 18px;margin: 0 0 11px;color: #444;">
                                                        {{ substr(strip_tags($post->title), 0, 40 ) }}{{ strlen(strip_tags($post->title)) > 40 ? "..." : ""  }}
                                                    </p>

                                                    <p style="font-size: 14px;margin: 0 0 11px;color: #444; line-height: 22px;">
                                ...
                                <br />
                                <br />
                                @if(count($n_val->posts) > 5)
                                    <button class="btn news_load_button" data-cid="{{$n_val->id}}" data-count="5"> more </button>
                                @endif
    ...
    <div class="container" style="padding-top: 80px; padding-bottom: 80px;">
        @if(count($movies) > 0)
            <div class="contain movies_slider">
                <h2 align="center" style="margin: 3px; color: #fff; font-family: 'Open Sans';">Watch Movies Now</h2>
