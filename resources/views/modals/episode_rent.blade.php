            ...
            <div class="modal-body" style="background-color:#fff; padding-left:50px; padding-right:50px;">
                <div class="row form-row">
                    <div class="col-md-12 text-center">
                        <span style="font-size: 30px;">{{ $episode->title }}</span> <span>{{ $episode->year }}</span>
                        ...
                        <div class="col-md-12">
                            @if(Auth::user()->card_last_four && Auth::user()->braintree_token && Auth::user()->braintree_id)
                                <form method="post" action="{{ route('srented') }}">
                                    {{ csrf_field() }}
                                    <input id="amount" name="amount" type="hidden" min="1"  value="{{ $episode->price }}">
                                    <input name="cd__36698__1WWWm" type="hidden" min="1"  value="{{ $episode->id }}">
                                    <div class="modal-footer m-t-10" style="background-color:#fff; border-top:0px solid #333; padding: 0px;">
                                        <button id="submitButton" value="{{ $episode->price }}" name="submitsavepayment" type="submit" class="btn btn-danger btn-block">Select saved payment method: <b>************{{Auth::user()->card_last_four}}</b></button>
                                    </div>
                                </form>
                                <hr>
                            @endif
                            ...
                            <form id="form-pay" method="post" action="{{ route('rented') }} ">
                                ...
                                <div class="modal-footer m-t-10" style="background-color:#fff; border-top:0px solid #333; padding: 0px;">
                                    <button id="submitButton" value="{{ $episode->price }}" name="submit" type="submit" class="btn btn-danger btn-block">RENT $ {{ $episode->price }} </button>
