            ...
            <div class="modal-body" style="background-color:#fff; padding-left:50px; padding-right:50px;">
                <div class="row form-row">
                    <div class="col-md-12 text-center">
                        <span style="font-size: 30px;">{{ $season->title }}</span> <span>{{ $season->year }}</span>
                        ...
                        <div class="col-md-12">
                            @if(Auth::user()->card_last_four && Auth::user()->braintree_token && Auth::user()->braintree_id)
                                <form method="post" action="{{ route('srented') }}">
                                    {{ csrf_field() }}
                                    <input id="amount" name="amount" type="hidden" min="1"  value="{{ $season->price }}">
                                    <input name="cd__39165__1WWWm" type="hidden" min="1"  value="{{ $season->id }}">
                                    <div class="modal-footer m-t-10" style="background-color:#fff; border-top:0px solid #333; padding: 0px;">
                                        <button id="submitButton" value="{{ $season->price }}" name="submitsavepayment" type="submit" class="btn btn-danger btn-block">Select saved payment method: <b>************{{Auth::user()->card_last_four}}</b></button>
                                    </div>
                                </form>
                                <hr>
                            @endif
                            ...
                            <form id="form-pay" method="post" action="{{ route('rented') }} ">
                                ...
                                <div class="modal-footer m-t-10" style="background-color:#fff; border-top:0px solid #333; padding: 0px;">
                                    <button id="submitButton" value="{{ $season->price }}" name="submit" type="submit" class="btn btn-danger btn-block">RENT $ {{ $season->price }} </button>
