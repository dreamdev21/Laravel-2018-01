<form action="{{ route('tvvideocreate') }}" method="post" id="tv_add_vid_form" enctype="multipart/form-data">
    <div class="modal-header">
        ...
        <h4 id="myModalLabel" class="semi-bold">Add new stream</h4>
        <br>
    </div>
    <div class="modal-body">
                                ...
                                <div class="time_controls col-lg-6 col-md-8 col-sm-8 ">
                                    <div class="row">
                                        <div class="col-xs-3"><input type="number" name="video_length_h" min="0" id="video_length_h"
                                                                     placeholder="H"
                                                                     class="form-control">
                                            <p style="color: red" class="hidden" id="error_h">This field cannot be empty</p>
                                        </div>
                                        ...
                                        <div class="col-xs-3"><input type="number" min="0" max="60" name="video_length_m" id="video_length_m"
                                                                     placeholder="M"
                                                                     class="form-control">
                                            <p style="color: red" class="hidden" id="error_m">This field cannot be empty</p>
                                        </div>
                                        <div class="col-xs-1 time_dots"><b>:</b></div>
                                        <div class="col-xs-3"><input type="number" name="video_length_s" min="0" max="60" id="video_length_s"
                                                                     placeholder="S"
                                                                     class="form-control">
                                            <p style="color: red" class="hidden" id="error_s">This field cannot be empty</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
        ...
        <script>
            ...
            $('#tv_add_vid_form').on('submit', function(){
                var url = ytVidId($('#url_valid').val());
                if(!url) {
...
<script>
    $('form#tv_add_vid_form').submit(function () {
        ...
        if($('input[name="video_length_h"]').val() === ""){
            hide();
            $('#error_h').removeClass('hidden');
            return false;
        }
        if($('input[name="video_length_m"]').val() === ""){
            hide();
            $('#error_m').removeClass('hidden');
            return false;
        }
        if($('input[name="video_length_s"]').val() === ""){
            hide();
